...
function getGeocodedAddress(address) {
    ...
    var GEOCODE_URL_BASE = "https://maps.googleapis.com/maps/api/place/textsearch/json?"
    var CITY = config.GOOGLE_GEOCODE_LOCATION
    var COUNTRY = "US"
    var timer = Date.now();
    return new Promise((resolve, reject) => {
        var querry = encodeURIComponent(address + " Anchorage, Alaska")
        request(`${GEOCODE_URL_BASE}query=${querry}&location=61.2181,-149.9003&radius=50000&key=${process.env.GOOGLE_PLACES_KEY}`, function(error, response, body){
