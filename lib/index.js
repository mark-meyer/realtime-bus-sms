...
function getStopFromStopNumber(stopId) {
    ...
    .then((muniBusData)=>{ 
        var parsedData = parseBusData(muniBusData.data, stopId)
        return {data: parsedData, muniTime:muniBusData.asyncTime}
    })
...
* @returns {Promise} - resolves to {data:raw-html-text, asyncTime:timedRepsonsforHttpCall}. 
* Rejects with error if we can't connect or recieve non 2xx response.
* 
*/
    ...
    return new Promise((resolve, reject) => {
        ...
        var request = http.get(config.MUNI_URL + busTrackerId, (response) => {
            if (response.statusCode < 200 || response.statusCode > 299) {
                reject(new Error('Failed to load bustracker, status code: ' + response.statusCode)); //should this just me a message or do we need the stack too?
           }
          const body = [];
...
function getGeocodedAddress(address) {
    ...
    return geocoder.geocode(address)
        ...
        .catch((err) => logger.error("Google Geocode Error:", err))
}
